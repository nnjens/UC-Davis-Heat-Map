classdef arduino < handle
    ...
    methods
        ...
        % Added by Stanley Hsu on 1/2012 for ENG6 
        % function potentiometer write (pWrite)
        
        
        function pWrite(a,pot_num,val)
           ...
           %%%%%%%%%%%%%%%%%%%%%%%% ARGUMENT CHECKING %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if a.chkp,
                ...
                % check nargin
                if nargin~=3,
                    error('Function must have the "pot_num" and "val" arguments');
                end
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % send mode, pin and value
                if pot_num == 0  % potentiometer 0
                    fwrite(a.aser,[53 val],'uchar');
                end
                
                if pot_num == 1  % potentiometer 1
                    fwrite(a.aser,[54 val],'uchar');
                end
                
                if pot_num == -1 % BOTH potentiometers
                    fwrite(a.aser,[55 val],'uchar');
                end
